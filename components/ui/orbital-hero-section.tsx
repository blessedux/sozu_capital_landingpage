...
export interface OrbitalHeroSectionProps
  ...
  paused?: boolean;
  sunColor?: string;
  /** Canvas clear / host fill. Defaults to pure black. */
  ...
  useEffect(() => {
    ...
    function buildStars() {
      starN = Math.max(
        60,
        Math.round(
          props.current.starCount * Math.min(2, (width * height) / (1440 * 900))
        )
      );
    ...
    function render(t: number) {
      ...
      for (let s = 0; s < starN; s++) {
        ...
        let a =
          (0.2 + sMag[s] * 1.05) * Math.pow(dRef / P.depth, 0.8) * near * far;
        if (a <= 0.012) continue;
        ...
        const size = Math.min(
          2.3,
          0.55 + sMag[s] * 1.5 * Math.pow(dRef / P.depth, 0.5)
        );
      ...
      let idx = 0;
      while (idx < shots.length && shots[idx].depth > camDist)
        drawShot(shots[idx++]);
      drawSun(k, t);
      ...
      if (C.scrim !== "none") {
        ...
        const [sr, sg, sb] = parseRGB(C.backgroundColor);
        const g =
          C.scrim === "left"
        ...
        for (let q = 0; q <= 12; q++) {
          ...
          g.addColorStop(
            x,
            `rgba(${sr},${sg},${sb},${(s * Math.pow(1 - x, 2.4)).toFixed(4)})`
          );
        }
        ctx!.fillStyle = g;
        ctx!.fillRect(0, 0, width, height);
