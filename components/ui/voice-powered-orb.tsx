...
const DEFAULT_BASE: [number, number, number][] = [
  [0.611765, 0.262745, 0.996078],
  [0.298039, 0.760784, 0.913725],
  [0.062745, 0.078431, 0.6],
];

const ORANGE_BASE: [number, number, number][] = [
  [1.0, 0.501961, 0.0],
  [1.0, 0.705882, 0.235294],
  [0.721569, 0.188235, 0.0],
];
...
const FRAG = /* glsl */ `
  ...
  vec4 draw(vec2 uv) {
    vec3 color1 = uDirectColor > 0.5 ? uBase1 : adjustHue(uBase1, hue);
    vec3 color2 = uDirectColor > 0.5 ? uBase2 : adjustHue(uBase2, hue);
    vec3 color3 = uDirectColor > 0.5 ? uBase3 : adjustHue(uBase3, hue);
...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const palette = tone === "orange" ? ORANGE_BASE : DEFAULT_BASE;
      program = new Program(glContext, {
        ...
        uniforms: {
          ...
          hover: { value: 0.15 },
          rot: { value: 0 },
          hoverIntensity: { value: 0.25 },
          uBase1: { value: new Vec3(...palette[0]) },
          uBase2: { value: new Vec3(...palette[1]) },
          uBase3: { value: new Vec3(...palette[2]) },
          uDirectColor: { value: tone === "orange" ? 1 : 0 },
        },
      });
